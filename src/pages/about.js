...
module.exports = function about(t, L) {
  ...
  /* Sections describing the company only appear once there is confirmed
     copy for them — an empty block is worse than no block. */
  const storyBlock = a.story.body.length
    ? `<section class="story">
  ...
  const pillarsBlock = a.pillars.items.length
  ...
  const guidesBlock = a.guides.body.length
  ...
  const safetyBlock = a.safety.items.length
...
<section class="mission">
  <div class="mission__inner">
    ...
    ${a.mission.body ? `<p class="mission__b reveal">${a.mission.body}</p>` : ''}
